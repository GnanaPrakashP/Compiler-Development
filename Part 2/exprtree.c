...
void update_field_func(char *name){
    struct classtable *temp=clookup(name);
    if(temp==NULL){
        printf("Parent class not defined to inherit\n");
        exit(0);
    }
    struct fieldlist *ptr1=temp->memberfield;
    while(ptr1!=NULL){
        struct fieldlist *new=(struct fieldlist*)malloc(sizeof(struct fieldlist));
        new->varname=(char*)malloc(strlen(ptr1->varname)*sizeof(char));
        strcpy(new->varname,ptr1->varname);
        new->fieldindex=ptr1->fieldindex;
        new->type=ptr1->type;
        new->ctype=ptr1->ctype;
        new->next=NULL;
        if(fieldlist_head==NULL){
            fieldlist_head=new;
        }
        else{
            struct fieldlist *tptr=fieldlist_head;
    ...
    struct memberfunclist *ptr2=temp->vfuncptr;
    while(ptr2!=NULL){
        struct memberfunclist *new=(struct memberfunclist*)malloc(sizeof(struct memberfunclist));
        new->varname=(char*)malloc(strlen(ptr2->varname)*sizeof(char));
        strcpy(new->varname,ptr2->varname);
        new->type=ptr2->type;
        new->paramlist=ptr2->paramlist;
        new->funcposition=ptr2->funcposition;
        new->flabel=ptr2->flabel;
        new->function_definition_flag=1;
        new->next=NULL;
        if(memberfunclist_head==NULL){
            memberfunclist_head=new;
        }
        else{
            struct memberfunclist *tptr=memberfunclist_head;
            while(tptr->next!=NULL){
                tptr=tptr->next;
            }
            tptr->next=new;
...
struct memberfunclist *mlookup(char *name){
...
struct memberfunclist *minstall(char *name, struct typetable *type){
    if(mlookup(name)!=NULL){
        struct memberfunclist *temp=mlookup(name);
        if(temp->type!=type){
            printf("Function type mismatch(Overriden and parent class function) in line %d\n",error_line);
            exit(0);
        }
        struct plist *ptr1=temp->paramlist;
        struct plist *ptr2=param_head;
        while(ptr1!=NULL&&ptr2!=NULL&&strcmp(ptr1->varname,ptr2->varname)==0&&ptr1->type==ptr2->type){
            ptr1=ptr1->next;
            ptr2=ptr2->next;
        }
        if(ptr1!=NULL||ptr2!=NULL){
            printf("Error in function arguments(overriden and parent class function) in line %d\n",error_line);
            exit(0);
        }
        temp->flabel=f_label;
        f_label++;
...
void check_pre_parent(char *name, struct classtable *ptr){
    while(ptr->parentptr!=NULL){
        ptr=ptr->parentptr;
        struct fieldlist *temp=ptr->memberfield;
        while(temp!=NULL){
            if(strcmp(temp->varname,name)==0){
                printf("Redeclaration of variabe(parent and child class) in line %d\n",error_line);
                exit(0);
            }
            temp=temp->next;
        }
...
struct fieldlist *finstall(struct typetable *type, struct classtable *ctype, char *name){
    check_pre_parent(name,clookup(class_name));
    struct fieldlist *new=(struct fieldlist*)malloc(sizeof(struct fieldlist));
    new->varname=(char*)malloc(strlen(name)*sizeof(char));
    strcpy(new->varname,name);
...
struct tnode *makeOperatorNode(int type, char *name, int nodetype, struct tnode *left, struct tnode *right){
    if(type==0){
        ...
        if(strcmp(name,"=")==0){
            if(strcmp(right->varname,"new")==0){
                // new operator
                struct classtable *temp=classcalculate(left);
                struct classtable *ptr=clookup(right->left->varname);
                while(ptr!=NULL){
                    if(ptr==temp){
                        break;
                    }
                ...
                if(ptr==NULL){
                    printf("Error(The assigned class is not the same or a child class) in line %d\n",error_line);
                }
            }
...
int getlabel(char *name, struct classtable *temp){
...
int offset(struct tnode *head){
    struct classtable *temp=classcalculate(head->left);
    /*while(temp!=NULL&&temp->next!=NULL&&temp->next->parentptr!=NULL){
        temp=temp->next;
    }*/
    int off_set=0;
    struct memberfunclist *ptr=temp->vfuncptr;
    while(ptr!=NULL){
        if(strcmp(ptr->varname,head->right->varname)==0){
            return off_set;
        }
        off_set++;
        ptr=ptr->next;
    ...
    if(head->left==NULL&&head->right==NULL&&head->nodetype!=10){
            ...
            if(strcmp(head->varname,"initialize")==0){
                ...
                if(object_flag){
                    ...
                    if(head->Ggentry==NULL){
                        struct classtable *tptr=classtable_head;
                        while(tptr!=NULL){
                            if(getlabel(head->varname,tptr)!=-1){
                                break;
                            }
                            tptr=tptr->next;
                        }
                        fprintf(fptr, "CALL F%d\n",getlabel(head->varname,tptr));
                    }
                    else{
                        fprintf(fptr, "CALL F%d\n",head->Ggentry->flabel);
                    }
        ...
        if(head->varname[0]=='.'){
            ...
            if(head->left==NULL){
                ...
                if(head->right->nodetype==10){
                    // Check weather obj.function obj class
                    // has that definition
                    struct classtable *cptr=classcalculate(head->left);
                    struct memberfunclist *ptr=cptr->vfuncptr;
                    while(ptr!=NULL&&strcmp(ptr->varname,head->right->varname)!=0){
                        ptr=ptr->next;
                    }
                    if(ptr==NULL){
                        printf("The function %s is not defined in the class %s\n",head->right->varname,cptr->varname);
                        exit(0);
                    }
                    if(ptr->function_definition_flag==1){
                        printf("The function %s is not defined in the class %s\n",head->right->varname,cptr->varname);
                        exit(0);
                    }
        ...
        if(strlen(head->varname)==1){
            switch(head->varname[0]){
                ...
                case '=':
                    fprintf(fptr, "MOV [R%d], R%d\n",x1,y2);
                    if(head->left->ctype!=NULL){
                        // Espacially for class objects
                        // 1) obj = new (A)
                        // 2) obj1 = obj2
                        fprintf(fptr, "ADD R%d, 1\n",x1);
                        if(head->right->nodetype==13){
                            int t=4096+8*(clookup(head->right->left->varname)->class_index);
                            fprintf(fptr, "MOV [R%d], %d\n",x1,t);
                        }
                        else{
                            fprintf(fptr, "ADD R%d, 1\n",x2);
...
void execute_virtual_table(FILE *fptr){
    int x=getReg();
    ...
    while(temp!=NULL){
        ...
        struct memberfunclist *ptr=temp->vfuncptr;
        while(ptr!=NULL){
            fprintf(fptr, "MOV [R%d], F%d\n",y,ptr->flabel);
            fprintf(fptr, "ADD R%d, 1\n",y);
            ptr=ptr->next;
        }
        freeReg(y);
        fprintf(fptr, "ADD R%d, 8\n",x);
        temp=temp->next;
    }
    freeReg(x);
    return;
...
void check_exist_function(char *name){
    ...
    if(temp->function_definition_flag==0){
        printf("Function %s is defined multiple times\n",name);
        exit(0);
    }
    else{
        temp->function_definition_flag=0;
    }
    struct plist *ptr1=temp->paramlist;
